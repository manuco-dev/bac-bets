...
const crearMlb = async (req, res = response) => {
    try {
        // Validación de datos
        ...
        const apuesta = new Mlb(req.body);
        await apuesta.save();
...
const listarMlb = async (req, res) => {
    try {
        // Obtención de las apuestas
        const listar = await Mlb.find({});
...
const editarBet = async (req, res, next) => {
    try {
        ...
        const updatedBet = await Mlb.findOneAndUpdate({ _id: id }, req.body, {
...
const eliminarBet = async (req,res = response)=>{
    try {
        const { id } = req.params;
        let bet = await Mlb.findByIdAndDelete( id );
